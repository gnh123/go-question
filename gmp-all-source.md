...
总结: 
go的调度模型就是生产者-消费者模型, go func是把g放到队列里面，消费者是个死循环不停从队列消费g

## 生产者
startm->newm->newm1->start0->start1->schedule

newproc->newproc1->gfget->如果是空就new一个新的，如果有值直接初始化newg，然后是runqput
runqput->runqputslow 
## 消费者

该函数的next为true, runqput就是把值先赋值到runnext, 老的runnext为空就结束runqput这个流程，然后存放到本地队列，如果没有空间就存到先构造一个batch数据，然后放到全局队列里面

findRunnable的流程， 也即schedule里面获取g的流程，大约分为7步

1.首先全局队列如果有数据，大约六十分之一的概率，从全局队列弹出一个g。如果这时候的全局队列还有一个g。把这个g放到这个p下面的runq里面。有g返回。无g继续往下走。  
2.P.runnext  
3.本地队列，从当前p的本地runq获取g。  
4.全局队列, 的runq获取g。  
5.netpoll 无阻塞调试一次, 超时被设置为0。  
6.窃取别的p的runq里面g。  
7.netpoll的g, timeout > 0  
...
top:
  ...
  // 1/61的概率，直接从全局队列里面取一个值
	if _p_.schedtick%61 == 0 && sched.runqsize > 0 {
		lock(&sched.lock)
    // 取一个g, 如果全局队列里面还有值，塞一个到局部队列里面
...
// 如果 next 是false, 就放到runq队列的尾部
// 如果 next 是true, 就放到_p.runnext
// runq队列满了，就放到全局队列中
// 疑问:
// 对tail的操作，只有一个go程？这里为加cas修改runqtail
// TODO 回答
...
// Executed only by the owner P.
// 如果runnext有值，就拿runnext
// 如果runnext取不到，就从要地runq里面取
...
// 从nextrun或者runq里面窃取数据到当前的runq里面
...
// not be used.
// 直接把q2的指针给到q
// 如果q是空队列，直接把q2的head, tail指针给到q
// 如果q不是空队列，直接把q2的head，等于q的tail.schedlink
